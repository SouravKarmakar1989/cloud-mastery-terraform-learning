# Extraction: certified-kubernetes-application-developer / 02_Core Concepts / 030_Services
...
- Lecture: 030_Services
- File: 030_Services.txt
- Topic: Services
...
Content:
First, if we were to SSH into the Kubernetes node at 192.168.1.2, from the node, we would be able to access the pods web page by doing a curl, or if the node has a GUI, we would fire up a browser and see the web page in a browser following the address.
