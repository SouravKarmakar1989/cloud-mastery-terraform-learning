# Extraction: certified-kubernetes-administrator-with-practice-tests / 03_Scheduling / 065_Resource Requirements
...
- Lecture: 065_Resource Requirements
- File: 065_Resource Requirements.txt
...
Content:
However, because limits are not set when available, any pod can consume as much memory as available, and if pod two requests more memory to free up pod one, the only option available is to kill it now, because unlike CPU, we cannot throttle memory once memory is assigned to a pod.
